...
| `Student_Lesson_WhenControlsBreakDown.html` | Digital Lesson | 5-section online module students complete before class. Covers the cell cycle, checkpoints, mutation mechanisms, cancer biology, and closes with a 3-question exit ticket. Includes Google Sheets submission integration for teacher data collection. |
...
| `Student_Guided_Notes_Lesson2.html` | Interactive Notes | Fully typeable 7-section note-catcher spanning the complete lesson arc. Auto-saves to browser. Includes **6 collapsible ELL sentence starters** at key write fields for language support. Print button produces a clean PDF with all typed answers preserved. |
| `Tumor_Board_Lesson2.html` | Group Activity | In-class peer discussion activity where student groups act as Maya's clinical Tumor Board. Four specialist roles (Molecular Biologist, Pathologist, Oncologist, Patient Advocate) each analyze the case from a different lens, then complete a group consensus form. Includes ELL sentence starters per role and Bloom's-labeled debrief questions. |
| `Scaffolded_Diagram_Lesson2.html` | Scaffolded Support | Interactive cell cycle diagram with pre-labeled phases and callout chips for IEP/504 and support students. Includes checkpoint descriptions, brake/accelerator analogy fill-ins, two-hit hypothesis write fields, and a self-check. Designed for discreet distribution during the in-class session. |
| `Extension_Activity_Lesson2.html` | Extension Activity | Data analysis extension for early finishers. Students compare TP53 mutation rates across pediatric ALL vs. adult cancer types using a real data table and 4 Bloom's-escalating analysis questions with a link to cBioPortal. |
...
| `Teacher_Classroom_Deck_Lesson2.pptx` | Projector Deck | 10-slide in-class presentation deck. Covers warm-up from exit ticket triage through the Maya case study analysis, Tumor Board debrief, research connection video clip, synthesis chain, and closing exit ticket. Bloom's level labeled on every activity slide. |
| `Design_Rationale_Document.docx` | Leave-Behind | 6-page document explaining the theoretical and empirical foundation behind every design decision — from the objective's Bloom's level to the structure of the simulator scenarios to why Maya appears in every document. |
...
- [Tumor Board Group Activity](https://lashaudracox.github.io/cell-division-gone-wrong/Tumor_Board_Lesson2.html)
- [Scaffolded Cell Cycle Diagram](https://lashaudracox.github.io/cell-division-gone-wrong/Scaffolded_Diagram_Lesson2.html)
- [Extension Activity: TP53 Across Cancers](https://lashaudracox.github.io/cell-division-gone-wrong/Extension_Activity_Lesson2.html)
...
| **2** ★ | **When Controls Break Down** | **Digital lesson + simulator + exit ticket** | **"Meet Maya" ALL case study + Tumor Board** | **Understand → Analyze** |
...
| Flipped model with exit ticket triage | Formative Assessment Theory | Harris & Jones, 2020 |
...
| ELL sentence starters — collapsible frames in guided notes and Tumor Board roles | Scaffolding for PBL | Belland, Kim & Hannafin, 2013 |
...
| Tumor Board activity — structured peer collaboration on Maya case | Social Constructivism / ARCS Satisfaction | Vygotsky, 1978; Keller, 1987 |
| 3-level exemplar responses in facilitation guide | Formative Assessment | Harris & Jones, 2020 |
| Exit ticket data captured via Google Sheets for real-time teacher triage | Formative Assessment Theory | Harris & Jones, 2020 |
...
- **Language translation:** All 8 student-facing HTML files include an embedded Google Translate widget (floating button, top-right corner). Supports 130+ languages — no API key or setup required. Students select their language from the dropdown and the entire page translates automatically.
- **Exit ticket data collection:** The digital lesson exit ticket can submit responses to a teacher-owned Google Sheet via Google Apps Script. See setup instructions in the exit ticket source file.

---

## Setting Up Exit Ticket Data Collection

The exit ticket in `Student_Lesson_WhenControlsBreakDown.html` can automatically send student responses to a Google Sheet. One-time setup:

1. Create a new Google Sheet and go to **Extensions → Apps Script**
2. Paste the `doPost()` function from the comments inside the HTML file
3. Deploy as **Web App** (Execute as: Me · Who can access: Anyone)
4. Copy the deployment URL and paste it into `const SUBMIT_URL = ''` in the file
5. Push the updated file to GitHub

Each submission adds a row with: timestamp, student name, class period, Q1 answer + correctness, Q2 answer + correctness, and short answer text.
...
- Harris, L., & Jones, M. (2020). *Measuring student learning.* Assessment Reform Group.
